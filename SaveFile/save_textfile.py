...
class Spider(object):
    ...
    def save_txt(self):
        file = open('hotnews.txt','w',encoding='utf-8')
        for item in self.infolist:
            rank =item[0]
            link =item[1]
            title = item[2]
            # file.write(rank+'\t'+link +"\t"+title+'\n')
            file.write('\t'.join([rank, link, title]))
            file.write('\n' + '=' * 100 + '\n')
        file.close()
    ...
    def save_json(self):
        ...
        file = open('hotnews.json', 'w', encoding='gbk')
        file.write(json.dumps(newList,indent=2,ensure_ascii=False))
        file.close()

    def save_csv(self):
        file = open('hotnews.csv','w',newline='')
        writer = csv.writer(file)
        for item in self.infolist:
            rank = item[0]
            link = item[1]
            title = item[2]
            writer.writerow([rank, link, title])
